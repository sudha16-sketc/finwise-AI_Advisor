...
// Remove: const USER_ID = 'user_demo_001'
...
export default function Analyze() {
  ...
  // Remove: const [userId, setUserId] = useState(USER_ID)

  const handleSubmit = async () => {
    if (!user) { setError('You must be logged in.'); return }  // ← add
    ...
    setResult(null)
    try {
      const response = await finwiseApi.analyze({ user_id: user.email, financial_text: text })  
      setResult(response)
    } catch (err) {
              ...
              <ul className="space-y-2">
                {result.advice.tax_saving_suggestions.map((s, i) => (
                  <li key={i} className="flex gap-3 text-sm text-slate-700">
                    <span className="w-6 h-6 rounded-full bg-amber-100 text-amber-700 text-xs flex items-center justify-center font-bold shrink-0 mt-0.5">{i + 1}</span>
                    {s}
                  </li>
                ))}
              </ul>
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6">
              ...
              <ul className="space-y-2">
                {result.advice.income_growth_suggestions.map((s, i) => (
                  <li key={i} className="flex gap-3 text-sm text-slate-700">
                    <span className="w-6 h-6 rounded-full bg-sky-100 text-sky-700 text-xs flex items-center justify-center font-bold shrink-0 mt-0.5">{i + 1}</span>
                    {s}
                  </li>
                ))}
              </ul>
